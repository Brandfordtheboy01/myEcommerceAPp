...
export function VendorInventory({ products }: VendorInventoryProps) {
  const inStock = products.filter((p) => p.stock > 10).length;
  const lowStock = products.filter((p) => p.stock > 0 && p.stock <= 10).length;
  const outOfStock = products.filter((p) => p.stock === 0).length;
  const totalProducts = products.length;

  // Calculate percentage and segments
  const availablePercent = totalProducts > 0 ? Math.round((inStock / totalProducts) * 100) : 100;
  const gaugeSegmentCount = 32;

  const inStockSegments = totalProducts > 0 ? Math.round((inStock / totalProducts) * gaugeSegmentCount) : gaugeSegmentCount;
  const lowStockSegments = totalProducts > 0 ? Math.round((lowStock / totalProducts) * gaugeSegmentCount) : 0;
