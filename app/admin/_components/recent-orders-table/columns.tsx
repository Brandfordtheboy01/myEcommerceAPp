...
export const recentOrdersColumns: ColumnDef<OrderRow>[] = [
    ...
    cell: ({ row }) => (
      <div className="flex flex-col gap-0.5">
        ...
        <div className="text-muted-foreground text-xs">
          {row.original.vendorCount} vendor{row.original.vendorCount !== 1 ? "s" : ""}
        </div>
      </div>
    ),
    enableHiding: false,
  ...
  },
  {
    ...
    filterFn: (row, _columnId, value) => {
      if (value === "Needs action") return row.original.payment !== "Paid";
      if (value === "Unpaid") return row.original.payment === "Pending";
      if (value === "Returns") return row.original.payment === "Refunded";
      return true;
    },
  },
  {
    accessorKey: "total",
    header: () => <div className="w-28">Order Total</div>,
    cell: ({ row }) => <div className="w-28 tabular-nums">{row.original.total}</div>,
  },
  {
    accessorKey: "commission",
    header: () => <div className="w-28">Commission</div>,
    cell: ({ row }) => (
      <div className="w-28 tabular-nums font-medium text-green-700 dark:text-green-400">
