...
export function Inventory({ inStock, lowStock, outOfStock }: InventoryProps) {
  const totalProducts = inStock + lowStock + outOfStock;
  const availablePercent = totalProducts > 0 ? Math.round((inStock / totalProducts) * 100) : 100;
  const inStockSegments = Math.round((inStock / (totalProducts || 1)) * gaugeSegmentCount);
  const lowStockSegments = Math.round((lowStock / (totalProducts || 1)) * gaugeSegmentCount);
